    ...
    async def execute(self, current_user: User):
        async with self.__tm:
            ...
            if current_user_membership is None:
                raise NoMembershipError("Provided user has no membership")
            elif current_user_membership.terminated:
                raise MembershipTerminatedError("User membership terminated")
            elif current_user_membership.is_suspended:
                raise MembershipSuspendedError("User membership suspended")
