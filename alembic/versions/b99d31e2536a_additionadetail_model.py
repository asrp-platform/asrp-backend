...
def upgrade() -> None:
    op.create_table('additional_details',
    ...
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], name=op.f('fk_additional_details_user_id_users')),
    sa.PrimaryKeyConstraint('id', name=op.f('pk_additional_details')),
    sa.UniqueConstraint('user_id', name=op.f('uq_additional_details_user_id'))
    )


def downgrade() -> None:
    op.drop_table('additional_details')
